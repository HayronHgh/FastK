use crate::chunk::header::TRADE_SCHEMA_ID;
use crate::error::{FastKError, Result};

use super::{read_i64, FixedRecord, RecordType};

/// Fixed-width trade tick record.
#[repr(C)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct TradeRecord {
    pub ts: i64,
    pub recv_ts: i64,
    pub trade_id: i64,
    pub price: i64,
    pub qty: i64,
    pub side: i8,
    pub flags: u8,
    pub _pad: [u8; 6],
}

impl TradeRecord {
    pub const BYTE_SIZE: usize = 48;

    pub fn to_le_bytes(&self) -> [u8; Self::BYTE_SIZE] {
        let mut out = Vec::with_capacity(Self::BYTE_SIZE);
        self.encode_le(&mut out);
        out.try_into()
            .expect("trade record encoder must emit BYTE_SIZE bytes")
    }

    pub fn from_le_bytes(bytes: [u8; Self::BYTE_SIZE]) -> Self {
        Self::decode_le(&bytes).expect("fixed-size trade bytes should decode")
    }

    pub fn validate_strict_order(records: &[Self]) -> Result<()> {
        <Self as FixedRecord>::validate_strict_order(records)
    }
}

impl FixedRecord for TradeRecord {
    const BYTE_SIZE: usize = TradeRecord::BYTE_SIZE;
    const SCHEMA_ID: u32 = TRADE_SCHEMA_ID;
    const RECORD_TYPE: RecordType = RecordType::Trade;
    const ALLOW_EQUAL_TIMESTAMPS: bool = true;

    fn ts(&self) -> i64 {
        self.ts
    }

    fn encode_le(&self, out: &mut Vec<u8>) {
        for value in [self.ts, self.recv_ts, self.trade_id, self.price, self.qty] {
            out.extend_from_slice(&value.to_le_bytes());
        }
        out.push(self.side as u8);
        out.push(self.flags);
        out.extend_from_slice(&self._pad);
    }

    fn decode_le(bytes: &[u8]) -> Result<Self> {
        if bytes.len() != Self::BYTE_SIZE {
            return Err(FastKError::InvalidData(format!(
                "unexpected trade record byte length: {}",
                bytes.len()
            )));
        }
        let mut offset = 0usize;
        let ts = read_i64(bytes, &mut offset)?;
        let recv_ts = read_i64(bytes, &mut offset)?;
        let trade_id = read_i64(bytes, &mut offset)?;
        let price = read_i64(bytes, &mut offset)?;
        let qty = read_i64(bytes, &mut offset)?;
        let side = bytes[offset] as i8;
        offset += 1;
        let flags = bytes[offset];
        offset += 1;
        let mut pad = [0u8; 6];
        pad.copy_from_slice(&bytes[offset..offset + 6]);
        Ok(Self {
            ts,
            recv_ts,
            trade_id,
            price,
            qty,
            side,
            flags,
            _pad: pad,
        })
    }
}

#[cfg(test)]
mod tests {
    use super::TradeRecord;

    #[test]
    fn trade_record_binary_roundtrip_preserves_fields() {
        let record = TradeRecord {
            ts: 100,
            recv_ts: 101,
            trade_id: 42,
            price: 123_456,
            qty: 789,
            side: -1,
            flags: 7,
            _pad: [0; 6],
        };

        assert_eq!(std::mem::size_of::<TradeRecord>(), TradeRecord::BYTE_SIZE);
        assert_eq!(TradeRecord::from_le_bytes(record.to_le_bytes()), record);
    }
}
